...
function fmtDefault( embed, sorted, globals, position, page = 1 ){
	let fields = [];
	for( let h = 0; h < ENTRIES_PER_PAGE/ENTRIES_PER_PANEL; h++ ){
		let value = "";
		sorted.slice( (page - 1 + h) * ENTRIES_PER_PANEL, ( page - 1 + h ) * ENTRIES_PER_PANEL + ENTRIES_PER_PANEL ).map( ( lbData, i )=>{
...
module.exports = function( lToken, globals, page = 1 ){
	...
	fmtter( embed, sorted, globals, position, page );
