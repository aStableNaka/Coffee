...
const tierAdjectives = [
	...
	['iron', 'copper', 'metal', 'polished', 'fine', 'strapped', 'grey', 'red', 'blue', 'cyan', 'orange', 'yellow', 'purple', 'green', 'strong', 'sturdy', 'pleasant'],
	...
	['ravenging', 'recursive', 'godly', 'flowing', 'holy', 'radiant', 'resonant', 'molecular', 'nano_tech', 'ethereal', 'ephemeral', 'faze', 'monotech', 'nakatech', 'juicy', 'ultimate', 'mad', 'king_killing', 'overthrowing', 'fancy', 'everlasting', 'kingly', 'demonic', 'hackers', 'doped', 'draconic']
];
const tierNouns = [
	['pickaxe'],
	['pickaxe', 'shovel', 'hand_drill'],
	['pickaxe', 'power_drill', 'shovel'],
	['pickaxe', 'power_drill', 'shovel', 'laser_drill', 'hole_maker', 'digging_unit', 'digger', 'hole_driller', 'ground_smasher', 'decimator', 'zero_divider']
];
...
class ItemPickaxe extends Item {
	constructor() {
		...
		this.emoji = "<:pickaxe:631407099729018900>";
	...
	skin(itemData){
		return locale.pickaxe.skins[(itemData.meta||{imgIndex:0}).imgIndex];
	}

	customEmoji(itemData){
		return this.skin.emoji;
	}
	...
	createItemData(amount, meta, name, tier = 0) {
		...
		return {
			...
			amount: amount,
			name: meta.name || name || this.accessor,
			meta: meta
		}
	}
	...
	ensureUserHasDefaultPickaxe(userData) {
		if ((!userData.hasFirstPickaxe && userData.pickaxe_accessor == "shifty_pickaxe") || (Object.values(userData.items).filter((x) => {
				return x.equipped;
			}).length == 0)) {
	...
	use(Chicken, itemData) {
		...
		itemData.amount--;
		itemData.equipped = true;
		console.log(itemData, unequippedPickItemData);
		Chicken.send(`You've swapped your ${ ufmt.item(unequippedPickItemData) } for your ${ ufmt.item(itemData) }`)
	}
