...
class CommandServer extends Command{
	...
	get helpPage(){ return 3; }
	  ...
	  executeHexCreate( Chicken, validColorInteger ){
			if(!Chicken.userData.hexRoleIDS){Chicken.userData.hexRoleIDS = {};}
			...
			if(existingRole){
				  ...
				  } ).catch((e)=>{ this.handleHexInvalidPerms(Chicken); });
			}else{
				  // create a new role
				  Chicken.guild.createRole({name:rolename,color:hexString}).then( (role)=>{
				  ...
				  }).catch(()=>{ this.handleHexInvalidPerms(Chicken); });
			}
	  }
